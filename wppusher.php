...
register_activation_hook( __FILE__, [ $deployer, 'activate' ] );
...
$hidePluginsFromUpdateChecks = function( $args, $url ) use ( $deployer ) {
	...
	foreach ( $pluginsToHide as $plugin ) {
		...
		unset( $plugins['active'][ array_search( $plugin, $plugins['active'], true ) ] );
	}
...
$hideThemesFromUpdateChecks = function( $args, $url ) use ( $deployer ) {
	...
	foreach ( $themesToHide as $theme ) {
		...
		if ( isset( $themes['active'] ) && in_array( $themes['active'], $themesToHide, true ) ) {
			unset( $themes['active'] );
		}
	}
...
if ( isset( $_GET['wpdeployer-welcome'] ) && $_GET['wpdeployer-welcome'] === '0' ) {
