...
class Deployer implements Container {
	...
	public function init() {
		add_action( 'init', [ $this->make( 'Deployer\Dispatcher' ), 'dispatchWebhookRequest' ] );
		add_action( 'admin_init', [ $this, 'registerPluginActionLinks' ] );
		add_action( 'admin_init', [ $this, 'registerSettings' ] );
		add_action( 'admin_init', [ $this->make( 'Deployer\Dispatcher' ), 'dispatchPostRequests' ] );
		...
		add_action( 'init', [ $this, 'nagBitbucketUsers' ] );
	}
	...
	public function adminMenu() {
		add_menu_page( $this->getName(), $this->getName(), 'manage_options', 'wpdeployer', [ $this->make( 'Deployer\Dashboard' ), 'getIndex' ], 'dashicons-marker' );
		add_submenu_page( 'wpdeployer', 'Install Plugin', 'Install Plugin', 'manage_options', 'wpdeployer-plugins-create', [ $this->make( 'Deployer\Dashboard' ), 'getPluginsCreate' ] );
		add_submenu_page( 'wpdeployer', 'WP Deployer Plugins', 'Plugins', 'manage_options', 'wpdeployer-plugins', [ $this->make( 'Deployer\Dashboard' ), 'getPlugins' ] );
		add_submenu_page( 'wpdeployer', 'Install theme', 'Install theme', 'manage_options', 'wpdeployer-themes-create', [ $this->make( 'Deployer\Dashboard' ), 'getThemesCreate' ] );
		add_submenu_page( 'wpdeployer', 'WP Deployer Themes', 'Themes', 'manage_options', 'wpdeployer-themes', [ $this->make( 'Deployer\Dashboard' ), 'getThemes' ] );
	}
	...
	public function registerSettings() {
		...
		add_filter( 'pre_update_option_wpdeployer_token', [ $this->make( 'Deployer\Services\TokenGenerator' ), 'refreshTokenFilter' ], 10, 2 );

		register_setting( 'deployer-license-settings', 'wpdeployer_license_key' );
		add_filter( 'pre_update_option_wpdeployer_license_key', [ $this->make( 'Deployer\License\LicenseManager' ), 'activateSiteLicense' ], 10, 2 );

		register_setting( 'deployer-gh-settings', 'gh_token', [ $this, 'checkGhToken' ] );
		register_setting( 'deployer-bb-settings', 'bb_token', [ $this, 'checkBbToken' ] );
		register_setting( 'deployer-bb-settings', 'bb_user' );
		register_setting( 'deployer-bb-settings', 'bb_pass', [ $this, 'checkBbPass' ] );
		register_setting( 'deployer-gl-settings', 'gl_base_url' );
		register_setting( 'deployer-gl-settings', 'gl_private_token', [ $this, 'checkGlToken' ] );
		register_setting( 'deployer-enable-logging', 'deployer_logging_enabled' );

		add_filter( 'pre_update_option_bb_token', [ $this, 'removeBitbucketCredentialsWhenTokenIsAdded' ], 10, 2 );
	}
	...
	public function addAction( $tag, $handler, $priority = 10, $acceptedArgs = 1 ) {
		...
		add_action(
			$tag,
			function( $action ) use ( $handler, $deployer ) {
